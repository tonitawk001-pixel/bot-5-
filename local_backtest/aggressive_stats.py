"""Aggressive bot stats - print trade frequency."""
...
df = pd.read_csv(os.path.join(DATA_DIR, "XAUUSD_1y_H1.csv"))
...
ms = 35; tp_mult = 6.0; sl_mult = 1.5; trail_mult = 0.5; max_pos = 2; TP_TREND = 6.0
...
print("="*60)
print("  AGGRESSIVE BOT — BACKTEST SUMMARY (1 Year H1)")
print("="*60)
...
print(f"  Profit Factor: {abs(sum(win_pnls)/sum(loss_pnls)):.2f}")
...
print(f"  Avg trades/day:   {len(closed)/trading_days:.2f}" if trading_days>0 else "  Avg trades/day: N/A")
vals=list(daily_trades.values())
if vals:
    print(f"  Busiest day:      {max(vals)} trades")
    print(f"  Quietest day:     {min(vals)} trades")
print(f"  Calendar months:  12")
print(f"  Avg trades/month: ~{len(closed)//12}")

print(f"\n{'='*60}")
print(f"  LOT SIZE SCALING EXPLANATION")
print(f"{'='*60}")
print(f"  Balance $0-$500:    2.0% risk/trade")
print(f"  Balance $500-$2K:   2.5% risk/trade")
print(f"  Balance $2K-$10K:   3.0% risk/trade")
print(f"  Balance $10K+:      4.0% risk/trade")
print(f"")
print(f"  Example $305 bal:   ~0.02 lots (2% of $305/SL dist)")
print(f"  Example $1,000 bal: ~0.07 lots")
print(f"  Example $10,000 bal:~1.0 lots ")
print(f"  Example $100,000 bal: ~5.0 lots")