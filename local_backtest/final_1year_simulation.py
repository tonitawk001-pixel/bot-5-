...
- M15 entry cycle
- M1 position management (trailing, break-even)
- Exact risk tiers, halt rules, and Friday close.
"""
...
# ATR Multipliers (from main_super.py)
TP_ATR_MULT = 4.0
...
def load_data():
    ...
    m15_file = os.path.join(data_dir, "XAUUSD_1y_M15.csv")
    m5_file = os.path.join(data_dir, "XAUUSD_1y_M5.csv")
    m1_file = os.path.join(data_dir, "XAUUSD_1y_M1.csv")

    print("Loading data files...")
    df15 = pd.read_csv(m15_file, index_col='time', parse_dates=True)
    df5 = pd.read_csv(m5_file, index_col='time', parse_dates=True)
    
    if os.path.exists(m1_file):
        df1 = pd.read_csv(m1_file, index_col='time', parse_dates=True)
        print(f"  M1 data loaded: {len(df1)} bars")
    else:
        df1 = None
        print("  M1 data NOT found. Using M5 for management.")
...
def main():
    ...
    start_time = df15.index[500] 
    current_cycle = 0
    for ct, row in master_df.iterrows():
        current_cycle += 1
        if current_cycle % 10000 == 0:
            print(f"  Processed {current_cycle} minutes... Balance: ${balance:.2f}")
        ...
        if ct.weekday() == 4 and ct.hour >= 21:
            if positions:
                for pos in positions:
                    ...
                    balance += pnl
                    pos['exit_time'] = ct; pos['exit_price'] = exit_price; pos['pnl'] = pnl; pos['reason'] = 'FridayClose'
        ...
        if positions:
            ...
            for pos in positions:
                if pos['type'] == 'BUY':
                    if row['low'] <= pos['sl']:
                        pnl = (pos['sl'] - pos['entry']) * CONTRACT_SIZE * pos['lots']
                        balance += pnl
                        pos['exit_time'] = ct; pos['exit_price'] = pos['sl']; pos['pnl'] = pnl; pos['reason'] = 'SL'
                        closed_trades.append(pos); daily_pnl += pnl; consecutive_losses += 1 if pnl < 0 else 0
                        if pnl >= 0: consecutive_losses = 0
                    elif row['high'] >= pos['tp']:
                        pnl = (pos['tp'] - pos['entry']) * CONTRACT_SIZE * pos['lots']
                        balance += pnl
                        pos['exit_time'] = ct; pos['exit_price'] = pos['tp']; pos['pnl'] = pnl; pos['reason'] = 'TP'
                        closed_trades.append(pos); daily_pnl += pnl; consecutive_losses = 0
                    else:
                    ...
                    if row['high'] >= pos['sl']:
                        pnl = (pos['entry'] - pos['sl']) * CONTRACT_SIZE * pos['lots']
                        balance += pnl
                        pos['exit_time'] = ct; pos['exit_price'] = pos['sl']; pos['pnl'] = pnl; pos['reason'] = 'SL'
                        closed_trades.append(pos); daily_pnl += pnl; consecutive_losses += 1 if pnl < 0 else 0
                        if pnl >= 0: consecutive_losses = 0
                    elif row['low'] <= pos['tp']:
                        pnl = (pos['entry'] - pos['tp']) * CONTRACT_SIZE * pos['lots']
                        balance += pnl
                        pos['exit_time'] = ct; pos['exit_price'] = pos['tp']; pos['pnl'] = pnl; pos['reason'] = 'TP'
                        closed_trades.append(pos); daily_pnl += pnl; consecutive_losses = 0
                    else:
        ...
        if last_m15_time is None or m15_time > last_m15_time:
            ...
            try:
                ...
                idx5 = df5.index.get_indexer([ct], method='pad')[0]
                m5_window = df5.iloc[idx5-50:idx5+1]
                ...
                if direction != "NONE" and score >= MIN_SCORE:
                    ...
                    if direction == "BUY":
                        entry_price = price + (SPREAD_POINTS * 0.005)
                        sl = entry_price - sl_dist; tp = entry_price + tp_dist; be_trigger = entry_price + (atr_val * BE_ATR_MULT)
                    else:
                        entry_price = price - (SPREAD_POINTS * 0.005)
                        sl = entry_price + sl_dist; tp = entry_price - tp_dist; be_trigger = entry_price - (atr_val * BE_ATR_MULT)
    ...
    out_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "final_simulation_results.csv")
...
if __name__ == "__main__":
    ...
    end_time = df15.index[-1]
    
    print(f"Starting simulation from {start_time} to {end_time}...")
    master_df = df1 if df1 is not None else df5
